...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkQuery = $con->prepare("SELECT id FROM cab_users WHERE username = ?");
    ...
    if ($checkQuery->num_rows > 0) {
        ...
        $insertQuery = $con->prepare("INSERT INTO cab_users (name, username, mobile, email, password) VALUES (?, ?, ?, ?, ?)");
        $insertQuery->bind_param("sssss", $name, $username, $mobile, $email, $hashedPassword);
        if ($insertQuery->execute()) {
            $message = "<div class='alert alert-success'>Signup successful. You can now <a href='user_login.php'>login</a>.</div>";
            $name = $username = $mobile = $email = $password = '';
        } else {
